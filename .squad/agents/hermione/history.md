...
- 2026-05-11T16:02:49.311+02:00: Delivered MVP onboarding slice with a 3-step New Project overlay (name → add local git repos via native picker → done), backed by real Tauri commands (`create_project`, `create_repo`, `create_session`, `open_chat_window`) and inline git-repo validation error handling from backend responses.
- 2026-05-11T16:02:49.311+02:00: Updated chat bootstrap to prefer an explicitly requested initial session id, so onboarding “Open Chat” lands in the newly created session with hydrated repo branch context on first render.
- 2026-05-11T16:02:49.311+02:00: Added onboarding helper tests in Vitest to lock core flow guards (project name validity, repo-step continuation gating, repo-name inference from picker paths).
